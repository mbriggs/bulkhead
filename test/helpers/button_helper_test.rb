...
class ButtonHelperTest < ActionView::TestCase
  ...
  test "solid button icons are white, non-solid icons are zinc" do
    assert_match(/class="[^"]*\bbutton-icon\b[^"]*\bleading\b/, button("Create", type: :primary, icon_name: :plus))
    assert_match(/class="[^"]*\bbutton-icon\b[^"]*\bleading\b/, button("Edit", type: :secondary, icon_name: :pencil))
  end
  ...
  test "shadow false omits shadow classes" do
    ...
    assert_includes classes, "button"
  end
  ...
  test "link type keeps button root for scoped link styles" do
    classes = button_classes(type: :link)
    assert_includes classes, "button"
